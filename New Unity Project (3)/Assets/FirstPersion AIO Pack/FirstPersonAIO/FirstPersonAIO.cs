...
public class FirstPersonAIO : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (useStamina)
        {//스테미너를 사용할 때
            isSprinting = Input.GetKey(sprintKey) && !isCrouching && staminaInternal > 0 && (Mathf.Abs(fps_Rigidbody.velocity.x) > 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z) > 0.01f);//대충 조건 맞고 달리기키 누르면 달리기
            if (isSprinting)//달리기고 있을때
            {
                staminaInternal -= (staminaDepletionSpeed * 2) * Time.deltaTime;//스테미나 초당 소모
                if (drawStaminaMeter)
                {//달릴때 스테미나 소모 보여주는거 켜면 스테미나 소모 그리기
            ...
            {//안달리고 쉴때
                staminaInternal += staminaDepletionSpeed * Time.deltaTime;//휴식중에 스테미나 회복
            }
            if (drawStaminaMeter)
            {
                ...
                StaminaMeter.transform.localScale = new Vector3(x, 1, 1);//스테미나 바 크기 설정
            }
